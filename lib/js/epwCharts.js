...
//initialization code for the dewpoint temperature floodplot
function epwDewPointFloodPlot(epw) {
    params = {};
    var value = [];
    if (unitSystem == "IP") {
        value = convertCtoF(epw.dewPointTemperature());
        params.unit = "\xB0F";
    } else {
        value = epw.dewPointTemperature();
        params.unit = "\xB0C";
    };
    var data = epwData(epw,value); //encoding most of the object construction here    
    params.id = "#epwDewPointFloodPlot";
    params.min_value = Math.min.apply(Math,value);
    params.max_value = Math.max.apply(Math,value);    
    params.steps = 7;
    params.step_colors = ['darkblue','blue', 'cyan', 'greenyellow', 'yellow', 'orange', 'red','darkred'];
